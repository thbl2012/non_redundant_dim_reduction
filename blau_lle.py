...
def blau_null_space(M, k, alpha=0.6, eigen_solver='arpack', tol=1E-6, max_iter=100,
                    random_state=None):
  ...
  if eigen_solver == 'arpack':
    ...
    try:
      ...
      for i in range(1, k):
        ...
        G = np.dot(temp, np.dot(M, temp))
        G -= (G.mean(axis=1)[:, np.newaxis] + G.mean(axis=0) - G.mean())
        eigval_temp, eigvect_temp = eigsh(G, 1, tol=tol)
    ...
    # Subsequent iterations
    # Set M = eigmax * I - M
    M = np.identity(M.shape[0], dtype=float) * eigmax - M
    ...
    for i in range(1, k):
      ...
      G = np.dot(temp, np.dot(M, temp))
      G -= (G.mean(axis=1)[:, np.newaxis] + G.mean(axis=0) - G.mean())
      eigval_temp, eigvect_temp = eigh(G, eigvals=(0, 0))
      eigen_values[i] = eigmax - eigval_temp[0]
      eigen_vectors[:, i] = eigvect_temp[:, 0]
      eigen_vectors[:, i] /= np.sqrt(np.sum(eigen_vectors[:, i] ** 2))
...
def blau_locally_linear_embedding(
  ...
  return blau_null_space(M, n_components, alpha=alpha, eigen_solver=eigen_solver,
                         tol=tol, max_iter=max_iter, random_state=random_state)


class BlauLocallyLinearEmbedding(
